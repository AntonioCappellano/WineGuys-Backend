...
const { sendOrderConfirmation, sendOwnerNotification } = require("../utils/mail");
    ...
    if (discount_code) {
      ...
      connection.query(sqlCoupon, [discount_code], (err, couponResults) => {
        ...
        if (serverTotal < coupon.min_spending) {
          return res.status(400).json({
            message: `Spesa minima dello sconto non raggiunta (${coupon.min_spending}€)`,
          });
        }
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`;
      ...
      connection.query(sqlCustomer, customerValues, (err, customerResult) => {
        if (err)
          return res.status(500).json({
            success: false,
            message: "Errore inserimento cliente",
            error: err.sqlMessage,
          });
          ...
          AND stock_quantity >= ? `;
              ...
              connection.query(sqlUpdateStock, [wine.quantity, wine.product_id, wine.quantity], (err, result) => {
                if (err && !errorSent) {
                  ...
                  return res.status(500).json({
                    success: false,
                    message: "Errore aggiornamento quantità",
                    error: err.sqlMessage,
                  });
                }
                if (result.affectedRows === 0 && !errorSent) {
                  ...
                  return res.status(400).json({
                    success: false,
                    message: `Quantità insufficiente per il prodotto ${wine.product_name}`,
                  });
                }
                ...
                if (wineUnit === cart_items.length && !errorSent) {
                  ...
                  const emailData = {
                    ...
                    subtotal: serverTotal.toFixed(2), // prezzo pieno
                    discount_code: discount_code || null,
                    total_price: price.toFixed(2), // prezzo scontato
                  ...
                  // funzione mail cliente con la conferma ordine
                  ...
                  setTimeout(() => {
                    sendOwnerNotification(emailData)
                      .then(() => console.log("Email gestore inviata correttamente"))
                      .catch((mailErr) => console.error("Errore invio email gestore:", mailErr));
                  }, 30000);
  ...
  connection.query(sqlCoupon, [discount_code], (err, results) => {
    ...
    if (!coupon) {
      return res.status(404).json({
        success: false,
        message: "Codice sconto non esistente o non valido",
      });
    }
    ...
    if (now < new Date(coupon.start_date) || now > new Date(coupon.end_date)) {
      return res.status(400).json({
        success: false,
        message: "Codice sconto scaduto o non ancora attivo",
      });
    }
